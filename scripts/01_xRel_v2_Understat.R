...
rlnt_all_1819_by_team %>% 
  ...
  select(1,8:13) %>% 
  arrange(desc(`xG_ahead_2_%`))

#get minutes per game state ----
seriea_1819_rlnt_min <- seriea_1819_rlnt %>%
  select(1:3,6,10:14) %>% 
  distinct() %>% 
  mutate(#HOME STATE AND MINUTE COUNTER
    state_h = case_when((goal_h == 1 & lead(sum_goal_h) > lead(sum_goal_a)) | sum_goal_h > sum_goal_a ~ "lead",
                             sum_goal_h == sum_goal_a ~ "level",
                             sum_goal_h < sum_goal_a ~ "behind"),
         min_ahead_h = case_when(state_h == "lead" & lag(state_h) == "level" | state_h == "lead" & minute == min(minute) ~ minute,
         ...
         lead_time_h = case_when(state_h == "lead" & minute == max(minute) ~ max(minute) - min_ahead_h,
           state_h == "level" & lag(state_h) == "lead" ~ min_ahead_h - lag(min_ahead_h),
                                 TRUE ~ 0),
        #AWAY STATE AND MINUTE COUNTER
    state_a = case_when((goal_a == 1 & lead(sum_goal_a) > lead(sum_goal_h)) | sum_goal_a > sum_goal_h ~ "lead",
                        sum_goal_a == sum_goal_h ~ "level",
                        sum_goal_a < sum_goal_h ~ "behind"),
    min_ahead_a = case_when(state_a == "lead" & lag(state_a) == "level" | state_a == "lead" & minute == min(minute) ~ minute,
...
rlnt_min_totals %>% filter(tot_min_ahead_h < 0 | tot_min_ahead_a < 0)

View(seriea_1819_rlnt_min %>% filter(match_id == 9596) %>% print(n = 33))
