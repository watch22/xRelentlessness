...
win_by_1 <- season_clean %>% 
  ...
  rbind(lei1516_df) %>% select(competition, season, match_id, minute, h_team, a_team, h_goals, a_goals) %>% 
  group_by(match_id) %>%
  filter(minute == max(minute)) %>% 
  distinct() %>% 
  mutate(GD = h_goals - a_goals,
         team = ifelse(GD > 0, h_team, ifelse(GD < 0,a_team,"draw")),
         by_1 = ifelse(abs(GD) == 1, 1, 0)) %>% 
  filter(GD != 0) %>% 
  group_by(team, season) %>% 
  summarise(across(where(is.numeric), ~ sum(.x))) %>% 
  arrange(desc(by_1)) %>% 
  select(team, season, by_1) %>% 
  head(20) %>% mutate(joiner = sub(" ","_",paste0(team,season))) %>% 
  ungroup()

win_by_1

left_join(win_by_1 %>% ungroup %>% select(joiner,by_1),
          pma_by_team_season %>% ungroup() %>% mutate(joiner = sub(" ","_",paste0(team,season))),
          by = "joiner") %>% 
  select(-1) %>% 
  select(team, season, Pos, by_1) %>% 
  rename(Team = team,
         Season = season,
         `League Position` = Pos,
         `Win by 1` = by_1) %>% 
...
left_join(win_by_1_top4 %>% ungroup %>% select(joiner,by_1),
          pma_by_team_season %>% ungroup() %>% mutate(joiner = sub(" ","_",paste0(team,season))),
          by = "joiner") %>% 
  select(-1) %>% 
  select(team, season, Pos, by_1) %>%
  ...
  select(team,season,Pos,total_xG_one_ahead,total_xGA_one_ahead, xG_one_ahead, one_ahead, xGA_one_ahead,norm_xG_one_ahead, norm_xGA_one_ahead, by_1) %>% 
  arrange(desc(norm_xGA_one_ahead))
  mutate(total_xGD_one_ahead = total_xG_one_ahead - total_xGA_one_ahead,
         xGD_per_one_ahead = xG_per_one_ahead - xGA_per_one_ahead,
         norm_xGD_one_ahead = norm_xG_one_ahead - norm_xGA_one_ahead) %>% 
  select(team,season,Pos,total_xGD_one_ahead,xGD_per_one_ahead,norm_xGD_one_ahead,by_1) %>% 
  arrange(desc(norm_xGD_one_ahead))
